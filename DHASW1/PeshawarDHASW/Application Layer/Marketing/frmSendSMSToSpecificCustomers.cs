...
{
    public partial class frmSendSMSToSpecificCustomers : Telerik.WinControls.UI.RadForm
    {
        public frmSendSMSToSpecificCustomers()
        {
            InitializeComponent();
        }
        ...
        private void DataLoading()
        {
            ...
            {
                new SqlParameter("@Task","MobileNoGettoSendSMS_SpecificCustomers"),
        ...
        private void SendSMS()
        {
            try
            {
                ...
                {
                    SMS_Mesage = "Dear Customer, " + "Dues of Rs." +Row.Cells["RemainingAmount"].Value.ToString() + " and surcharge of Rs." +
                    Row.Cells["BalanceSurcharge"].Value.ToString() + " is outstanding against your File No." + Row.Cells["FileNo"].Value.ToString() + 
                    " in DHA Peshawar.You are requested to clear your dues on priority basis to avoid further surcharge.";
                    if (Regex.IsMatch(Row.Cells["MobileNo"].Value.ToString(), MakeCombinedPattern()))
                    {
                        ...
                        status = SMSSEndingNEW(Row.Cells["MobileNo"].Value.ToString(), SMS_Mesage);
        ...
        private void btnDataReadyForSMS_Click(object sender, EventArgs e)
        {
            SqlParameter[] par = {
                                     ...
                                     new SqlParameter("@Is_SMS_Letter","SMS")
                                 };  
           Helper.SQLHelper.ExecuteDataset(Helper.SQLHelper.createConnection(),
                                                  CommandType.StoredProcedure,
                                                  "App.tbl_SMSSendingLog",
                                                  par);
