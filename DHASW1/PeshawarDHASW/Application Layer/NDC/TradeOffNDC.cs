...
{
    public partial class TradeOffNDC : Telerik.WinControls.UI.RadForm
    {
        ...
        private void btn_File_Verify_Click(object sender, EventArgs e)
        {
            ...
            SqlParameter[] prm =
            {
                new SqlParameter("@Task","CheckFileNoInTradeOffFortbl"),
                new SqlParameter("@FileNo",txtFile_No_.Text)
            };
            DataSet stdst = cls_dl_NDC.NdcRetrival(prm);
            if(Convert.ToUInt32(stdst.Tables[0].Rows[0]["TotalCount"].ToString()) > 0)
            {
                ...
                if (txtFile_No_.Text != ""  && plotdealerState)
                {
                ...
                else
                {
                    ...
                    MessageBox.Show("Please Enter TRADE OFF Detail.");
                }
      ...
      double osCharges, double undcFee, double rindcfee, double get_TotalDueRemaining, double stampDuty, string fileno, string requested_by)
        ...
        private void TradeOffNDC_Load(object sender, EventArgs e)
        {
            btn_Buyer.Text = "Add New Buyer";
            txtPhase_.Text = "Phase-1";
        ...
        private DataTable NDC_Table_()
        {
            ...
            try
            {
                ...
                row["NDCCategory"] = "TadeOff_BuybackNDC";
