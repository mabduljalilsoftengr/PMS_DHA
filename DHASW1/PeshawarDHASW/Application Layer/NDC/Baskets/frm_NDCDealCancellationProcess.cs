...
{
    public partial class frm_NDCDealCancellationProcess : Telerik.WinControls.UI.RadForm
    {
        public frm_NDCDealCancellationProcess()
        {
            InitializeComponent();
        }
        private int NDCNo { get; set; }
        private void frm_NDCDealCancellationProcess_Load(object sender, EventArgs e)
        {
            GetCancelledNDC();
        }
                                    ...
                                    "Are you want to save and go next.", "Attention !", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        for (int i = 0; i < grdImagePreview.RowCount; i++)
                        {
                            ...
                            {
                                     new SqlParameter("@Task", "InsertNDCRefundByTFR"),
        ...
        private void btngoback_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show("Are you want to move NDC in previous state ?","Attention!",MessageBoxButtons.YesNo,MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    SqlParameter[] prm =
                    {
                      new SqlParameter("@Task","GoBackToTFRAppBasket"),
                      new SqlParameter("@NDCNo",NDCNo),
                    ...
                    if (rslt > 0)
                    {
                        MessageBox.Show("NDC is Successfully moved to preivious state.");
                        GetCancelledNDC();
                    }
                }
