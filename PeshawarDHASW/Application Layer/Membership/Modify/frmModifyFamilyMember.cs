...
namespace PeshawarDHASW.Application_Layer.Membership.Modify
{
    public partial class frmModifyFamilyMember : Telerik.WinControls.UI.RadForm
    {
        ...
        public frmModifyFamilyMember(int ID, string fileNo)
        {
            MBSID = ID;
            FileNo = fileNo;
            InitializeComponent();
            ...
            try
            {
                ...
                if (IDFamilyID == 0)
                {
                            ...
                            == DialogResult.Yes)
                    {
                        ...
                        //bool rslt = clsPluginHelper.ApplicationLogSaving(FileNo, clsUser.Name, "Before - Modification Membership Family Record. ", parameters, "", "SQLParam");
                        if (result > 0)
                        {
                ...
                else
                {
                           ...
                           == DialogResult.Yes)
                    {
                        ...
                        int result = cls_dl_Membership.Membership_FamilyMember_NonQuery(parameters);
                        bool rslt = clsPluginHelper.ApplicationLogSaving(FileNo, Models.clsUser.ID + "-" + clsUser.Name + "-" + branch, "After - Modification Membership Family ", parameters, "", "SQLParam");
                        if (result > 0)
                        {
