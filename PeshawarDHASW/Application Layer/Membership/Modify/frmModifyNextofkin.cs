...
namespace PeshawarDHASW.Application_Layer.Membership.Modify
{
    public partial class frmModifyNextofkin : Telerik.WinControls.UI.RadForm
    {
        ...
        public frmModifyNextofkin(int ID, string fileNo)
        {
            MBSID = ID;
            FileNo = fileNo;
            InitializeComponent();
        ...
        private void btnSavechangesnextofkin_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                cls_dl_ModifyMembership.Modify_Membership_nextofkin(parameters);
                 ...
                 bool rslt = clsPluginHelper.ApplicationLogSaving(FileNo, Models.clsUser.ID + "-" + clsUser.Name + "-" + branch, "After - Modification Membership NextofKin Record. ", parameters, "frmModifyNextofkin - btnSavechangesnextofkin_Click", "SQLParam");
                this.Close();
            }
            catch (Exception ex)
            {
