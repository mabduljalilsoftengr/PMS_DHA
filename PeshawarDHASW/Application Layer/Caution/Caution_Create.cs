    ...
    public partial class Caution_Create : Telerik.WinControls.UI.RadForm
    {
        ...
        private void Caution_Create_Load(object sender, EventArgs e)
        {
            ...
            if(CautionID > 0)
            {
                ...
                bool result = clsPluginHelper.ApplicationLogSaving(txtFilePlotNo.Text, clsUser.ID + "-" + clsUser.Name + "-" + branch, "Before - Modification Caution Record.", dst, "Caution_Create - Caution_Create_Load", "DataSetTable");
        ...
        private void LoadCurrentMembers()
        {
            try
            {
                ...
                //bool rslt = clsPluginHelper.ApplicationLogSaving(txtFilePlotNo.Text, clsUser.Name, "Before - Modification Caution Records.", dst, "", "DataSetTable");
            }
            catch (Exception ex)
            {
        ...
        private void btnAddRemarks_Click(object sender, EventArgs e)
        {
            try
            {
                        ...
                        if(dst.Tables.Count > 0)
                        {
                            if(dst.Tables[0].Rows.Count > 0)
                            {
                                ...
                                else if(dst.Tables[0].Rows[0]["Remarks"].ToString() == "This File need to be Cautioned.")
                                {
                                    ...
                                    bool rslt1 = clsPluginHelper.ApplicationLogSaving(txtFilePlotNo.Text, Models.clsUser.ID + "-" + clsUser.Name + "-" + branch, "Insertion in Caution record. ", prmtr, "Caution_Create - btnAddRemarks_Click", "SQLParam");
                                    #endregion
                                }
                            }
                        }
                ...
                else
                {
                    ...
                    bool result = clsPluginHelper.ApplicationLogSaving(txtFilePlotNo.Text, clsUser.ID + "-" + clsUser.Name + "-" + branch, "After - Modification Caution record.", prmtr, "", "SQLParam");
           ...
           // bool rslt = clsPluginHelper.ApplicationLogSaving(txtfileno.Text, clsUser.Name, "Before - Modification in File No.", ds, "Select the File data before updateion", "DataSetTable");
        }
