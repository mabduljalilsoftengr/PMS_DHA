...
namespace PeshawarDHASW.Application_Layer.FileMap.SvcBenefitFiles
{
    public partial class frmSvcCreateFile : Telerik.WinControls.UI.RadForm
    {
                ...
                if (1==1) //ds.Tables.Count > 0)
            {
                if (1==1) //)
                {
                        try
                        {
                            ...
                            bool rslt = clsPluginHelper.ApplicationLogSaving(FileNo1, Models.clsUser.ID + "-" + clsUser.Name + "-" + branch, "Insertion - Record inserted in Svc File", parameters, "frmSvcCreateFile - btnSaveandprinttheschedule_Click", "SQLParam");
        ...
        private void btnAddPlotAllData_Click(object sender, EventArgs e)
        {
            ...
            frmAddPlotNoAllRecordData obj = new frmAddPlotNoAllRecordData(txtfileno.Text);
            obj.ShowDialog();
