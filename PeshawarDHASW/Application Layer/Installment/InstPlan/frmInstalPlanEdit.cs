...
namespace PeshawarDHASW.Application_Layer.Installment.InstPlan
{
    public partial class frmInstalPlanEdit : Form
    {
        ...
        private void FillDataGrid()
        {
            ...
            bool rslt = clsPluginHelper.ApplicationLogSaving(textBoxFileNo.Text, Models.clsUser.ID + "-" + clsUser.Name + "-" + branch, "Before - Modification Re-order plan.", ds, "frmInstalPlanEdit -", "DataSetTable");
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(clsMostUseVars.Connectionstring))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand("[App].[USP_InstallmentTemplate]", conn))
                    {
                        ...
                        bool rslt = clsPluginHelper.ApplicationLogSaving(textBoxFileNo.Text, Models.clsUser.ID + "-" + clsUser.Name + "-" + branch, "After - Modification of Plan Re-order", cmd, "frmInstalPlanEdit - btnUpdateInstallment_Click", " SQLCommand");
