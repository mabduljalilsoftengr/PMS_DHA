        ...
        private void Save_OR_Update()
        {
            ...
            try
            {
                if (instal_Templateid > 0)
                {
                    ...
                    if (PLANID > 0) // Update
                    {
                        bool rslt = clsPluginHelper.ApplicationLogSaving(fileNo, Models.clsUser.ID +"-" + clsUser.Name + "-" + branch, "After - Modification in Installment Plan ID : " + PLANID.ToString(), cmd, "frmInstPlanAdd_MOdify - Save_OR_Update", "SQLCommand");
                    }
                    else
                    {
                        bool rslt = clsPluginHelper.ApplicationLogSaving(fileNo, Models.clsUser.ID + "-" + clsUser.Name + "-" + branch, "Insertion in Installment Plan ID : " + PLANID.ToString(), cmd, "frmInstPlanAdd_MOdify - Save_OR_Update", "SQLCommand");
                    }
            ...
            try
            {
                ...
                if (PLANID > 0)
                {
                    ...
                    foreach (DataRow row in ds.Tables[0].Rows)
                    {
                     ...
                     bool rslt = clsPluginHelper.ApplicationLogSaving(fileNo, Models.clsUser.ID + "-" + clsUser.Name + "-" + branch, "Before - Modification in Installment Plan ID : " + PLANID.ToString(), ds, "frmInstPlanAdd_MOdify - frmInstPlanAdd_MOdify_Load", "DataSetTable");
