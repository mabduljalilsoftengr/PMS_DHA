...
namespace PeshawarDHASW.Application_Layer.Installment.InstReceive.InstReceiveModify
{
    public partial class frmDDModify : Telerik.WinControls.UI.RadForm
    {
        ...
        {
            //string fileNo = lblfileno.Text.Trim();
            try
            {
                ...
                string fileNo = ds_dt_for_mdf.Tables[0].Rows[0]["FileNo"].ToString();
                bool rslt = clsPluginHelper.ApplicationLogSaving(fileNo, Models.clsUser.ID + "-" + clsUser.Name + "-" + branch, "Before - Modification in Received installment ", ds_dt_for_mdf, "frmDDModify - btnsave_Click", "DataSetTable");
            ...
            try
            {
                if (_ReceID != 0)
                {
                  ...
                  //  cmd.Parameters.AddWithValue("@ChallanNo", string.IsNullOrEmpty(txtChallanNo.Text) ? clsPluginHelper.DbNullIfNullOrEmpty("") : txtChallanNo.Text);
                    ...
                    bool rslt = clsPluginHelper.ApplicationLogSaving(fileNo, Models.clsUser.ID + "-" + clsUser.Name + "-" + branch, "After - Modification in Received installment ", cmd, "frmDDModify - btnsave_Click", "SQLCommand");
                    #endregion
                    if (result > 0)
                    {
                        this.Close();
