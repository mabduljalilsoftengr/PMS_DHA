    ...
    public partial class frmUserInsertion : Telerik.WinControls.UI.RadForm
    {
        ...
        private void LoadUserData(int get_id)
        {
            try
            {
                ...
                string Decrypted_Password = cls_dl_User.Pass_Decrypt(ds.Tables[0].Rows[0]["password"].ToString(), "sblw-3hn8-sqoy19");
                txtPassword.Text = Decrypted_Password;
        ...
        private void BindData_With_Grid()
        {
            ...
            new SqlParameter("@Task", "Select")
                ...
                {
                    ddl_MultiColCombobox_Employee.MultiColumnComboBoxElement.EditorControl.DataSource = ds.Tables[0];
                    ddl_MultiColCombobox_Employee.DisplayMember = "Name"; 
                    ddl_MultiColCombobox_Employee.ValueMember = "ID";

                    foreach (GridViewDataColumn column in ddl_MultiColCombobox_Employee.MultiColumnComboBoxElement.Columns)
                    {
                        if (column.Name == "ID")
                        {
                            column.IsVisible = false;
                    ...
                    ddl_MultiColCombobox_Employee.SelectedIndex = -1;
                }
                else
                {
                    ddl_MultiColCombobox_Employee.DataSource = null;
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (UserID == 0)
                {
                    ...
                    string password_encrpt = "";
                    if (txtSecretCode.Text != string.Empty)
                    {
                        password_encrpt = cls_dl_User.Encrypt(txtPassword.Text, "sblw-3hn8-sqoy19");
                    }
                    #endregion
                    SqlParameter[] prm =
                    {
                        ...
                        new SqlParameter("@password",password_encrpt), //need to encrypt 
        ...
        private void ddl_MultiColCombobox_Employee_SelectedValueChanged(object sender, EventArgs e)
        {
            //try
            //{
            //    if (ddl_MultiColCombobox_Employee.SelectedValue != null &&
            //        ddl_MultiColCombobox_Employee.SelectedValue.ToString() != "")
            //    {
            //        int selectedId = int.Parse(ddl_MultiColCombobox_Employee.SelectedValue.ToString());
        ...
        private void ddl_MultiColCombobox_Employee_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (ddl_MultiColCombobox_Employee.SelectedValue != null &&
