...
namespace PeshawarDHASW
{
    public partial class FrmLogin : RadForm
    {
        ...
        // this is updated with encryptiion and decryption
        private void btnlogin_Click(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection con = new SqlConnection(MainConnections.ConnectionString_MainServer))
                {
                    ...
                    // Get the password from textbox
                    string inputPassword = txtpassword.Text;

                    string encryptionKey = ConfigurationManager.AppSettings["EncryptionKey"];

                   // string encryptionKey = "sblw-3hn8-sqoy19"; // Your encryption key

                    //// First, try with the plain text password (for existing users with plain text passwords)
                    ...
                    // If no results with plain text, try with encrypted password
                    //if (dt.Rows.Count == 0)
                    //{
                        // Try with encrypted password
                        string encryptedPassword = cls_dl_User.Encrypt(inputPassword, encryptionKey);
                        cmd.Parameters.AddWithValue("@password", encryptedPassword);
                        //cmd.Parameters["@password"].Value = encryptedPassword;
                   ...
                   // }
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        else if (clsUser.Status == "Suspended")
                        {
                            this.Hide();
        ...
        // old way without encryption Decryption
        //private void btnlogin_Click(object sender, EventArgs e)
        //{
