...
namespace PeshawarDHASW.Helper
{
    ...
    {
        // DataSetTable   SQLParam
        public static bool ApplicationLogSaving(string FileNo, string UserName, string ActionName,object Data, string Remarks,string type)
        {
            bool status = false;
            string obj = "";
            try
            {
                ...
                if(type == "DataSetTable")
                {
                    obj = JsonConvert.SerializeObject(Data);
                }
                else if(type == "SQLParam")
                {
                    // Convert SqlParameter[] to dictionary
                    var paramList = Data as SqlParameter[];

                    var dict = paramList.ToDictionary(
                        p => p.ParameterName,
                        p => p.Value == DBNull.Value ? null : p.Value
                    );

                    // Serialize dictionary to JSON properly
                    obj = JsonConvert.SerializeObject(dict, Formatting.Indented);
                }
                else  if (type == "SQLCommand")
                {
                    obj = SqlCommandToJson(Data as SqlCommand);
                }


                SqlParameter[] param =
                {
                      new SqlParameter("@Task","SaveAppLog")
                    , new SqlParameter("@FileNo",FileNo)
                    , new SqlParameter("@UserName",UserName)
                    , new SqlParameter("@ActionName",ActionName)
                    , new SqlParameter("@Data",obj)
                    , new SqlParameter("@Remarks",Remarks)
                };
               
                  int rslt = int.Parse( SQLHelper.ExecuteNonQuery(SQLHelper.createConnectionLogging(), CommandType.StoredProcedure, "dbo.USP_SaveAppLog", param).ToString());
        ...
        public static string SqlCommandToJson(SqlCommand cmd)
        {
            var cmdObj = new
            {
                CommandText = cmd.CommandText,
                CommandType = cmd.CommandType.ToString(),
                Parameters = cmd.Parameters.Cast<SqlParameter>().Select(p => new
                {
            ...
            return JsonConvert.SerializeObject(cmdObj, Formatting.Indented);
        }


        //public async Task<bool> SaveActivityLog(string Remarks, object Description, string CNIC, string Controller, string ActionResult)
        //{
        //    bool status = false;
        //    try
        //    {

        //        tbl_APISActivityLog activitylog = new tbl_APISActivityLog();

        //        activitylog.Description = JsonConvert.SerializeObject(Description);
        //        activitylog.CNIC_Identity = CNIC;
        //        activitylog.Controller = Controller;
        //        activitylog.ActionResult = ActionResult;
        //        activitylog.Remarks = Remarks;
        //        activitylog.EntryDatetime = DateTime.Now;
        //        pAYMIRLOGEntities.tbl_APISActivityLog.Add(activitylog);
        //        await pAYMIRLOGEntities.SaveChangesAsync();
        //        if (activitylog.ID > 0)
        //        {
        //            status = true;
        //        }
        ...
        #region Jazz SMS API 
        public static bool SMSSendingJazz(string MobileNo, string Message)
        {
            ...
            string Username = "03203485013";
            string Password = "Jazz@123";
            string From = "Biz Demo";
            string To = MobileNo;
            string SmsAPIUrl = "https://connect.jazzcmt.com/sendsms_url.html";
            ...
            try
            {
                using (var client = new WebClient())
                {
                    ...
                    byte[] responseBytes = client.UploadValues(SmsAPIUrl, "POST", formData);
                    jasonData = System.Text.Encoding.UTF8.GetString(responseBytes);

                    // Handle responses based on Jazz SMS API responses
                    if (jasonData.Contains("Message Sent Successfully!") == true)
                    {
                        SMSStatus = true;
                        jasonData = "SMS Sent Successfully";
                    }
                    else if (jasonData.Contains("Invalid Number") == true)
                    {
                        SMSStatus = false;
                        jasonData = "Invalid Number";
                    }
                    else if (jasonData.Contains("Invalid Parameters") == true)
                    {
                        SMSStatus = false;
                        jasonData = "Invalid Parameters";
                    }
                    else if (jasonData.Contains("Invalid Username or Password") == true)
                    {
                        SMSStatus = false;
                        jasonData = "Invalid Username or Password";
                    }
                    else if (jasonData.Contains("Invalid Short Code") == true)
                    {
                        SMSStatus = false;
                        jasonData = "Invalid Short Code";
                    }
                    else if (jasonData.Contains("Your Package Has Been Expire") == true ||
                             jasonData.Contains("Package Expired") == true)
                    {
                        SMSStatus = false;
                        jasonData = "Your Package Has Been Expire.";
                    }
                    else if (jasonData.Contains("Insufficient Balance") == true ||
                             jasonData.Contains("Insufficent Balance") == true)
                    {
                        SMSStatus = false;
                        jasonData = "Insufficent Balance";
                    }
                    else if (jasonData.Contains("Failed To Send SMS") == true ||
                             jasonData.Contains("Failed") == true)
                    {
                        SMSStatus = false;
                        jasonData = "Failed To Send SMS";
                    }
                    else if (jasonData.Contains("IP not authorized") == true ||
                             jasonData.Contains("Unauthorized") == true)
                    {
                        SMSStatus = false;
                        jasonData = "IP not authorized";
                    }
                    else if (jasonData.Contains("Mask not allowed!") == true)
                    {
                        SMSStatus = false;
                        jasonData = "Mask not allowed!";
                    }
                    else
                    {
                        // Handle any other response as failure
                        SMSStatus = false;
                        jasonData = "Failed To Send SMS";
                    }
                }
